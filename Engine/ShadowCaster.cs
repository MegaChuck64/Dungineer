...
/*
 * Credit: https://fadden.com/tech/ShadowCast.cs.txt
...
public static class ShadowCast
{
    ...
    /// <summary>
    /// Returns a 2d array of floats representing the distance squared from the given position on the map
    /// </summary>
    ...
    private static void ComputeVisibility(CellGrid grid, Point gridPosn, float viewRadius)
    {
    ...
    private static void CastLight(CellGrid grid, Point gridPosn, float viewRadius,
    ...
    {
        private Cell[,] distMap;

        public int W => distMap.GetLength(0);
        public int H => distMap.GetLength(1);

        public CellGrid(bool[,] map)
        {
            distMap = new Cell[map.GetLength(0), map.GetLength(1)];
            for (int x = 0; x < distMap.GetLength(0); x++)
            {
                for (int y = 0; y < distMap.GetLength(1); y++)
                {
                    distMap[x, y] = new Cell { IsWall = map[x, y], Distance = float.MaxValue };
                }
            }
        }
        ...
        {
            return distMap[x, y].IsWall;
        }
        public void SetLight(int x, int y, float distSqrd)
        {
            distMap[x, y].Distance = distSqrd;
        }
